...
from detailbase.utils import AverageMeter, get_root_logger
...
def eval(epoch, best_iou, model, dataloader, cfg, logger, writer, save_ckp=False):
    if gorilla.is_main_process():
        ...
        pious, sious, long_ious, long_sious, complex_ious, complex_sious = [], [], [], [], [], []
    ...
    for batch in dataloader:
        ...
        piou = result['piou']
        siou = result['siou']
        long_iou = result['long_iou']
        long_siou = result['long_siou']
        complex_iou = result['complex_iou']
        complex_siou = result['complex_siou']
        if gorilla.is_main_process():
            pious.extend(piou)
            sious.extend(siou)
            long_ious.extend(long_iou)
            long_sious.extend(long_siou)
            complex_ious.extend(complex_iou)
            complex_sious.extend(complex_siou)
            progress_bar.update()
    ...
    if gorilla.is_main_process():
        ...
        #with open('ious_both_stmn_scanrefer.json', 'w') as f:
        ...
        writer.add_scalar('val/smIOU', smiou, epoch)
        
        writer.add_scalar('val/l_miou', long_miou, epoch)
        writer.add_scalar('val/l_Acc_50', long_precision_half, epoch)
        writer.add_scalar('val/l_Acc_25', long_precision_quarter, epoch)
        writer.add_scalar('val/l_smiou', long_smiou, epoch)
        
        writer.add_scalar('val/c_miou', complex_miou, epoch)
        writer.add_scalar('val/c_Acc_50', complex_precision_half, epoch)
        writer.add_scalar('val/c_Acc_25', complex_precision_quarter, epoch)
        writer.add_scalar('val/c_smiou', complex_smiou, epoch)
