...
def save_single_instance(root, key, pred_pmask, all_pred_pmask=None):
    ...
    with open(osp.join(root,'predicted_masks', f'{key}.json'), 'w') as f:
        print(f'{key}.json')
        json.dump(pred_pmask, f)
    
    if all_pred_pmask is not None:
        with open(osp.join(root,'predicted_masks', f'{key}_all_pred_masks.json'), 'w') as f:
...
def save_pred_instances(root, name, keys, pred_pmasks):
    ...
    pool = mp.Pool()
    pool.starmap(save_single_instance, zip(roots, keys, pred_pmasks))
    pool.close()
    pool.join()
