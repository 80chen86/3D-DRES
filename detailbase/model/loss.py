...
class Criterion(nn.Module):
    ...
    def get_new_pad_mask(self, pad_mask, t2os):
        new_pad_mask = []
        for i in range(len(t2os)):
            for k in t2os[i].keys():
                new_pad_mask.append(pad_mask[i])
        new_pad_mask = torch.stack(new_pad_mask)
        return new_pad_mask
    
    def get_pos_mask(self, t2os, pos_loss):
        pos_masks = []
        for t2o in t2os:
            mask = torch.zeros((pos_loss.shape[1],)).to(pos_loss.device)
            for k in t2o.keys():
                mask[int(k)] = 1
            pos_masks.append(mask.bool())
        pos_masks = torch.stack(pos_masks)
        pos_masks_other = (pos_loss.sum(-1) > 0) ^ pos_masks
        return pos_masks, pos_masks_other
    ...
    def get_layer_loss(self, layer, aux_outputs, pad_masks, gt_spmasks):
        ...
        loss = (self.loss_weight[0] * mask_bce_loss + self.loss_weight[1] * mask_dice_loss + self.loss_weight[2] * score_loss)

        loss_out = {f'layer_{layer}_' + k: v for k, v in loss_out.items()}
        return loss, loss_out

    def forward(self, pred, gt_spmasks, t2os):
        ...
        pad_masks = self.get_new_pad_mask(pad_masks, t2os)
        tgt_padding = pad_sequence(gt_spmasks, batch_first=True)
